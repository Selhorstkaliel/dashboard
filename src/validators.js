...
class Validators {
    ...
    initializeSchemas() {
        return {
            ...
            userRegistration: Joi.object({
                ...
                role: Joi.string().valid('admin', 'representante', 'vendedor').required(),
                rep_id: Joi.number().integer().min(1).when('role', {
                    is: 'vendedor',
                    then: Joi.required(),
                    otherwise: Joi.optional()
                }),
                discount_value: Joi.number().min(0).max(100).default(0)
            }),
    ...
    validateUserRegistration(data) {
        return this.schemas.userRegistration.validate(data, { abortEarly: false });
    }
    ...
    validateProfileUpdate(data) {
        return this.schemas.profileUpdate.validate(data, { abortEarly: false });
    }
    ...
    validateFileUpload(file, allowedTypes = ['image/jpeg', 'image/png', 'image/jpg', 'application/pdf']) {
        const errors = [];
        ...
        if (file.size > 5 * 1024 * 1024) {
            errors.push('Arquivo muito grande. Máximo 5MB permitido');
        }
